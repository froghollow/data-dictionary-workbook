        ...
        for glue_table in glue_tables_list:
            ...
            try:
                ...
                response = glue.update_partition(
                    DatabaseName = glue_database_name,
                    TableName = glue_table['Name'] ,
                    PartitionValueList = [ partition_key ],
                    PartitionInput = {
                        'Values' : [ partition_key ],
                        'StorageDescriptor' : glue_partition_sd,
                        'Parameters' : glue_table['Parameters']
            ...
            print(glue_table['Name'], partition_key, status)
...
output_path = 's3://wc2h-dtl-prd-datawork/rmyers07/TDSX/PROCESSED/DETAILED_TOP_DATA_SOURCE/{}.raml'.format(glue_table_name)
